...
== Import data from sql file to sqlite3 database

cd to c:\sqlite3
$ sqlite3 c:\websites\kennel-management\db\development.sqlte3

inside of sqlite3 console

> .read C:\\temp\\mgt.hungkwang.com\\sqlite3-showdogs.sql

## LIKE syntax

d= Dog.where(Dog.arel_table[:name].matches('puppy%'))

#  SELECT `dogs`.* FROM `dogs` WHERE (`dogs`.`name` LIKE 'puppy%')

## NOT IN

l= Litter.where.not(dog_id: nil)

# SELECT `litters`.* FROM `litters` WHERE (`litters`.`dog_id` IS NOT NULL) 

## convert to hash for both d and l

dHash= d.map{|c|[c.id]}
=> [[32], [42], [43], [44], [45], [47], [48], [49], [50], [52], [53], [54], [57], [59]]

lHash = l.order(dog_id: :asc).map{|c|[c.dog_id]}
=> [[32], [42], [43], [44], [45], [47], [48], [49], [50], [52], [53], [54], [57], [58]]

dHash-lHash
=> [[59]]

## l=Litter.where(Litter.arel_table[:dog_id].not_eq(nil))
=> SELECT `litters`.* FROM `litters` WHERE (`litters`.`dog_id` IS NOT NULL)

Litter.where{dog_id != nil}

## Subquery

new_puppy = Dog.where(Dog.arel_table[:name].matches('puppy%')).where.not(id: Litter.where.not(dog_id: nil).select(:dog_id))

=>
SELECT `dogs`.* FROM `dogs` WHERE (`dogs`.`name` LIKE 'puppy%') AND (`dogs`.`id` NOT IN (SELECT dog_id FROM `litters` WHERE (`litters`.`dog_id` IS NOT NULL)))

[#<Dog id: 59, name: "puppy-test", call_name: "puppy-test", birthday: "2014-01-04", registra
tion_no: "", created_at: "2014-01-04 22:59:42", updated_at: "2014-01-04 22:59:42", sire_id: 8, dams_id: 3, color_id: 1,
gender: "Female", status_id: 1>]>

## How to express a NOT IN query with ActiveRecord/Rails?

http://stackoverflow.com/questions/4307411/how-to-express-a-not-in-query-with-activerecord-rails

## refactoring not_assigned_puppies into scope variables in Dog model
...
instead of Dog.where(Dog.arel_table[:name].matches('puppy%')).where.not(id: Litter.where.not(dog_id: nil).select(:dog_id)).map{|d| [d.call_name, d.id]}

## What is the difference between a Block, a Proc, and a Lambda in ruby? It is just ways of grouping code we want to run

## Difference between Blocks and Procs: Procs are objects, blocks are not

# Procs are "anonymous methods" (or nameless functions) containing code. They can be placed inside of variable and passed around like any other object or scalar value. They are created by Proc.new, lambda, and blocks



## Differences between Procs and Lambdas: Both Procs and Lambdas are Proc objects
...
# Preson.find(1,2,3) 	# returns an array for object with IDs in (1,2,3)

# Prson.find([1,2])		# returns an array for object with IDs in (1,2)

## passing arguments into scope

http://guides.rubyonrails.org/active_record_querying.html#scopes

scope :published, -> { where(published: true) }
scope :created_before, ->(time) { where("created_at < ?", time) } OR
